...
export default function HomePage() {
  ...
  // Guard: only show this page when the user explicitly clicked the
  // "Executive Dashboard" nav link (which sets exec_nav in sessionStorage).
  // Direct navigation, bookmarks, and browser session restoration all land
  // here without the flag — send them to /welcome instead.
  useEffect(() => {
    if (typeof window === "undefined") return;
    const intentional = sessionStorage.getItem("exec_nav");
    if (!intentional) {
      router.replace("/welcome");
      return;
    }
    sessionStorage.removeItem("exec_nav");
  }, [router]);
