        ...
        <div style={WARNING_BANNER_STYLE}>
          <div onClick={() => setBannerOpen(!bannerOpen)} style={{ padding: "12px 16px", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              ...
              <div>
                <div style={{ fontSize: "14px", fontWeight: 700, color: WARNING_TITLE_COLOR }}>{longOpen.length} position{longOpen.length > 1 ? "s" : ""} open for 60+ days</div>
                <div style={{ fontSize: "12px", color: WARNING_TITLE_COLOR, marginTop: "1px" }}>{longOpen.map((i) => `${i.position_title} (${daysSince(i.date_opened)}d)`).join(" · ")}</div>
              </div>
            </div>
              ...
              {longOpen.map((i) => (
                <div key={i.id} onClick={() => { setExpandedId(i.id); setBannerOpen(false); }} style={{ padding: "8px 16px 8px 48px", borderBottom: `1px solid ${COLOURS.TRACK}`, cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    ...
                    <div style={{ fontSize: "13px", color: COLOURS.SLATE }}>{i.department || "—"} · Opened: {formatDateUK(i.date_opened)}</div>
                  </div>
                  <span style={{ fontSize: "14px", fontWeight: 700, color: COLOURS.RED }}>{daysSince(i.date_opened)}d open</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: "14px", marginBottom: "14px" }}>
          ...
          {donutData.length > 0 && (
            <div style={{ border: `1px solid ${COLOURS.HAIRLINE}`, borderRadius: RADII.CARD, padding: "16px 20px", backgroundColor: COLOURS.CARD }}>
              <div style={{ fontSize: "10.5px", fontWeight: 500, color: COLOURS.SLATE, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "10px" }}>Recruitment Pipeline</div>
              ...
              <div key={item.id} style={{ borderBottom: `1px solid ${COLOURS.HAIRLINE}` }}>
                ...
                {isOpen && (
                  <div style={{ padding: "12px 16px", backgroundColor: COLOURS.CARD_ALT, borderTop: `1px solid ${COLOURS.HAIRLINE}` }}>
                    {item.notes && <div style={{ fontSize: "14px", color: COLOURS.SLATE, marginBottom: "8px" }}>Notes: {item.notes}</div>}
                    <div style={{ display: "flex", gap: "6px", alignItems: "center" }}>
                      ...
                      <select value={item.status} onChange={(e) => updateStatus(item.id, e.target.value)} style={{ padding: "5px 8px", border: `1px solid ${COLOURS.HAIRLINE}`, borderRadius: RADII.SM, fontSize: "14px" }}>
