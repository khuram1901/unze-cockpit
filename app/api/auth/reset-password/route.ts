...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    await supabase.auth.admin.createUser({
      email: normalised,
      password: `Pulse-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
      email_confirm: true,
    });
    ...
    await sendNotificationEmail({
      to: normalised,
      subject: "Password Reset - Unze Pulse Dashboard",
      heading: "Reset Your Password",
      body: `
        <p>Hi <strong>${memberName}</strong>,</p>
        <p>You requested a password reset for your Unze Pulse Dashboard account.</p>
