  ...
  try {
    ...
    if (type === "task_assigned" && taskId) {
      ...
      await sendNotificationEmail({
        ...
        whatsAppMessage: `New task assigned: ${task.description?.slice(0, 100)}. Priority: ${task.priority}. Check cockpit for details.`,
      });
    ...
    if (type === "escalation" && taskId) {
      ...
      await sendNotificationEmail({
        ...
        recipientName: memberName,
        whatsAppPhone: member.notify_whatsapp ? member.phone_e164 : null,
        whatsAppMessage: `Escalation: ${task.description?.slice(0, 100)}. Please check the cockpit and respond.`,
      });
