...
export async function POST(request: NextRequest) {
  try {
    ...
    const tempPassword = `Pulse-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    ...
    await sendNotificationEmail({
      to: email.trim(),
      subject: `Welcome to Unze Pulse Dashboard - Set Up Your Account`,
      heading: `Welcome ${displayName}`,
      body: `
        <p>You have been added to the <strong>Unze Pulse Dashboard</strong> as <strong>${role}</strong>.</p>
