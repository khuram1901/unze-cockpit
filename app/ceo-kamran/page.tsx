...
export default function KamranDashboardPage() {
  ...
  // Route guard: only Kamran (kamran@unze.co.uk) and main admin may access this page
  useEffect(() => {
    if (!ctxLoading && ctx) {
      if (!isSecondaryCEO(ctx) && !isMainAdmin(ctx)) {
        router.replace("/home");
      }
    }
  }, [ctxLoading, ctx, router]);
  ...
  async function loadExecutiveData(dateToView: string) {
    ...
    // Kamran's dashboard is IFPL-scoped — only load IFPL finance data
    const FINANCE_COMPANIES = COMPANIES.filter(c => c.id === IFPL_COMPANY_ID);
    ...
    // Investment/portfolio data not shown on Kamran's IFPL-scoped dashboard
    const computedInvestmentData: InvestmentSummary | null = null;

    // Pension data not shown on Kamran's IFPL-scoped dashboard
    const computedPensionSummary: { gbp: number; pkr: number; netGain: number; totalReturn: number; contributed: number; feesPaid: number } | null = null;


    // Folderit data not shown on Kamran's IFPL-scoped dashboard
    const computedFolderitSummary: { pendingApproval: number; companyInbox: number; hrInbox: number } | null = null;
    const computedFolderitCompanyBreakdown: { group_key: string; inbox_count: number; inbox_oldest_days: number | null }[] = [];
  ...
  useEffect(() => {
    ...
    async function loadDashboard() {
      ...
      // Cron health fetch removed — admin-only, not shown on Kamran's dashboard
      ...
      <main style={{ padding: isMobile ? "16px 20px" : "32px 40px", maxWidth: "100%", minWidth: 0, backgroundColor: CANVAS, fontFamily: "var(--font-sans, Inter, sans-serif)" }}>
        ...
        {allLoading ? (
          ...
          <>
            ...
            {/* Cron Health panel removed — admin-only, not shown on Kamran's dashboard */}
