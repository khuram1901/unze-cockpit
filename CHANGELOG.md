...
## 2026-08-30 (evening) — Phase 4 tech debt: JS aggregation → RPCs, select("*") purge, orphaned HR routes gone

**Stock module (migration 224):** all remaining JS aggregation moved into security-definer RPCs — `get_plant_authority_letters`, `get_authority_letter_lookup`, `get_po_letter_totals`, `get_letter_dispatched_totals`, `close_po_if_fully_dispatched`, `get_entry_allocation_totals`, `get_po_produced_totals`. Routes are thin wrappers; 204 lines of `.reduce()`/`.filter()` deleted. Dispatch business rules preserved exactly (letter caps, expiry block, dual-write untouched); RPC outputs cross-checked against `get_stock_summary` for all 230 letters and direct sums for all 21 POs — zero mismatches. Auto-close now also respects `ordered_40` (old JS never selected it). **Folderit (migration 225):** `get_folderit_overview` builds the whole Overview payload in one round-trip. **P&L uploads (migration 226):** restatement detection reads stored sums via `get_ifpl/rest/utpl_pnl_line_sums`; payload parsing stays in JS by design. **select("*") purge:** explicit column lists on 10 sites (task list's 48-column table, HR T&D detail ×3, HR tasks, balance-sheet prior/restatement reads ×3, authority-letters and production-allocations listings). Kept deliberately: `member_permissions` flag rows (×10 — explicit lists would silently drop future permission columns), full-table backup, legal-case + cash-sheet detail endpoints. **Orphaned HR routes deleted** (no callers): `hr/performance`, `hr/employee-performance`, `hr/eobi`, `hr/recruitment/*`; empty `performance_evaluations` + `hr_strategy_goals` tables dropped (migration 228, Khuram confirmed); `recruitment_*` tables kept for the FlowHCM sync. `widgetRegistry` hr_tabs keys resynced to the 8 live tabs (4 tabs previously couldn't be hidden per-user).

**Latent production bugs found and fixed during the audit:** (1) migration 149 (July's authority-letter Close/Reopen) was never applied — `closed_at`/`closed_by` didn't exist, the Close button had failed silently since July; applied now, `get_stock_summary` output verified hash-identical for all 6 plants. (2) `production_allocations` PATCH wrote a nonexistent `updated_at` — every allocation edit failed; write removed. (3) Folderit `lastSyncAt` was always null (route read `created_at`; column is `ran_at`). (4) `hr_td_sessions` was missing the three feedback columns from migrations 171/173 — T&D feedback sync was broken; migration 227 adds them. (5) The shared Folderit account (Baranh+HD) now counts its issues against both companies (was nondeterministic last-link-wins — Khuram approved).

**Security lockdown (migrations 229-230):** the Supabase advisor flagged every security definer function as publicly executable. Audited all 133 `rpc()` call sites: 64 browser-called RPCs keep EXECUTE for authenticated; the rest are server-only. Result: anon can execute 0 of 178 functions (was 133+); authenticated 63; service_role all. **Missing RPC created (migration 231):** `get_production_summary` was called by the home briefing and CEO dashboard but never existed in the DB — production % and daily target had been silently blank. Now returns actuals (31+36+45) for the date, the monthly target sum, and target/days-in-month.

The lockdown initially broke policy-gated table reads — functions referenced inside RLS policies execute as the logged-in user, so the Tasks page failed with "permission denied for function can_access_all_tasks". Hotfix migration 232 re-grants EXECUTE to authenticated for all 23 RLS helper functions (is_admin, can_access_all_tasks, get_user_role, ...); views and column defaults were checked the same way — none affected. Also fixed the letter Close/Reopen confirmation button reading "Delete".

Verification: tsc clean per module; RPC-vs-JS numeric parity proven in SQL before each route swap; live browser sweep after deploy — Stock (FIEDMC totals match DB exactly), Folderit Overview (7/31/171 all matching, Baranh+HD both showing the shared account's issues), Tasks (all KPIs + rows), HR (8 tabs, 1,164 active), Imperial P&L, Investments, Receivables, Manage POs incl. a full letter Close→Reopen roundtrip — the first time that button has worked since July. Seven commits.
