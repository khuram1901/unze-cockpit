-- Migration 244: Fix get_hod_team_performance
-- Removes broken Finance sub-HOD FlowHCM scoping (Shahida wasn't in FlowHCM;
-- reports_to = NULL means she only saw herself).
-- All HODs now see all active members of their department(s) — consistent with
-- every other department. Also adds Admin/CEO fallback so Khuram can test without
-- needing can_view_team_performance on his own record.
...
BEGIN
  ...
  --    CEO/Admin fallback: if no entry in hod_department_access, show all departments.
  ...
  IF v_depts IS NULL THEN
    -- Check if this is an Admin/CEO user — give them a full org view
    IF EXISTS (
      SELECT 1 FROM members
      WHERE lower(email) = lower(p_hod_email)
        AND role IN ('Admin', 'CEO')
    ) THEN
      v_dept_label := 'All Departments';
  ...
  -- 2. Task performance calculation
  WITH task_base AS (
    SELECT
      t.id                                                                        AS task_id,
