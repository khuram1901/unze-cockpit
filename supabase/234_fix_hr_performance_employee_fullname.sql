-- Migration 234: Fix HR performance RPCs to show full employee names
-- Root cause: task_base CTEs used m.name AS emp_name, but members.name often
-- stores only the first name. Fix uses first_name + last_name with m.name fallback.
-- Pattern: COALESCE(NULLIF(TRIM(first || ' ' || last), ''), name)
-- Consistent with migration 183 (chat_tables) and migration 108 naming pattern.
-- NOTE: m.name NOT ILIKE ANY(...) exclusion filters are intentionally left as
-- m.name — they filter system/template accounts by their legacy name field.
...
BEGIN
  ...
  task_base AS (
    SELECT
      ...
      COALESCE(NULLIF(TRIM(COALESCE(m.first_name,'') || ' ' || COALESCE(m.last_name,'')), ''), m.name) AS emp_name,
