-- Migration 235: Score completed tasks by submitted_at, not completed_at.
-- Employees are judged on when THEY submitted, not when the manager approved.
-- Falls back to completed_at for historical records that pre-date migration 234.

-- Only the is_on_time / is_late lines in task_base change across all 3 RPCs.
-- Everything else (self-gen filter, aggregation, scoring formula) is unchanged from migration 233.
...
BEGIN
  ...
  task_base AS (
    SELECT
      ...
      -- Score by submitted_at (when employee submitted); fall back to completed_at for old records
      (t.status = 'Completed'
